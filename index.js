...
function submitQuiz()
{
    ...
    radioButtons.forEach(radio => {
        const role = radio.getAttribute("data-role");
    ...
    for(const role in belbinRoles)
    {
        if(belbinRoles[role].score>highestScore){
            highestScoreRole = belbinRoles[role].name;
    ...
    if (myStarWarsCharacter) {
        resultContainer.innerHTML = `
            <div class='result'>
                <h2>Your Belbin Role: ${belbinRoles[highestScoreRole.toLowerCase()].name}</h2>
                <p>${belbinRoles[highestScoreRole.toLowerCase()].description}</p>
